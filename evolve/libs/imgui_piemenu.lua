...
local function BeginPieMenuEx(menuCtx)
	...
	if isKeyDown(menuCtx.m_iKey) then
...
local function BeginPiePopup(menuCtx, pName, iKey)
	iKey = iKey or 18
...
local function EndPiePopup(menuCtx)
	...
	if menuCtx.m_bClose or (not bItemHovered and not isKeyDown(menuCtx.m_iKey)) then
...
local function New(...)
	...
	return {
		BeginPiePopup = function(name, Key)
			return BeginPiePopup(menuContext, name, Key)
		end,
